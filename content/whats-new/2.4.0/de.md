---
version: "2.4.0"
publishedAt: "2026-06-21"
title: "Was ist neu in Intake 2.4.0"
summary: "Intake AI ist da"
coverImage: "./assets/cover.svg"
highlights:
  - Mahlzeiten mit Intake AI loggen
  - Mahlzeiten natürlich beschreiben
  - Mahlzeitenfotos analysieren
  - Nährwertlabels scannen
  - Eigenen API-Schlüssel nutzen
---

## Intake AI ist da

Intake 2.4.0 bringt Intake AI: eine schnellere Möglichkeit, echte Mahlzeiten in bearbeitbare Kalorien und Makros umzuwandeln.

Du kannst beschreiben, was du gegessen hast, ein Mahlzeitenfoto nutzen oder ein Nährwertlabel scannen. Intake AI erstellt daraus einen Vorschlag, den du prüfen, anpassen und in dein Tagebuch speichern kannst.

![sharing](assets/ai.mp4)

## Drei Wege zum Start

Beschreibe eine Mahlzeit in einem Satz, wenn Tippen am schnellsten ist. Das passt gut für Alltagsgerichte, Snacks und schnelle Korrekturen.

Nutze ein Foto, wenn sich die Mahlzeit leichter zeigen als erklären lässt. Intake AI kann Teller, Snacks oder Restaurant-Mahlzeiten schätzen und daraus bearbeitbare Nährwerte machen.

Scanne ein Nährwertlabel, wenn du ein verpacktes Produkt vor dir hast. Intake AI kann das Label lesen und die Werte vor dem Speichern vorausfüllen.

![sharing](assets/nutrition.mp4)

## Vor dem Speichern prüfen

KI-Ergebnisse sind Schätzungen und können falsch sein. Intake AI schreibt Werte nicht einfach still in dein Tagebuch.

Jedes Ergebnis bleibt bearbeitbar. Wenn etwas nicht stimmt, kannst du die Werte direkt ändern oder im Chat natürlich antworten, zum Beispiel mit einer anderen Portionsgröße oder einer fehlenden Zutat.

## Preise

Leider führt kein Weg daran vorbei, Intake AI als Abomodell anzubieten. Bei jedem Foto und jeder Unterhaltung mit der KI entstehen Kosten,
die sich nur über einen monatlichen oder jährlichen Betrag gegenfinanzieren lassen. Das Versprechen von Intake, "ohne Abo", bleibt weiterhin bestehen:
Für die Core-Funktionalität von Intake gilt weiterhin: einmal bezahlen, für immer nutzen. Ich hoffe, ihr versteht, dass ich kein Feature
bauen kann, bei dem ich am Ende Geld drauflegen muss, damit ihr es nutzen könnt.

## Eigenen API-Schlüssel nutzen

Du kannst Intake AI auch mit deinem eigenen OpenAI- oder Claude-API-Schlüssel verwenden. Hinterlege den Schlüssel in den Intake-Einstellungen, wähle ein unterstütztes Modell, und Intake sendet KI-Anfragen über diesen Anbieter, ohne dass du ein Intake-AI-Abo brauchst.

Die Bedingungen und nutzungsabhängigen Kosten des jeweiligen Anbieters gelten trotzdem. Intake kontrolliert diese Preise oder Abrechnungsregeln nicht.

Wichtig: Ein normales ChatGPT-Plus-, ChatGPT-Pro-, Claude-Pro- oder Claude-Max-Abo enthält keinen API-Schlüssel und kein API-Guthaben. API-Zugriff ist separat und benötigt einen Entwickler- beziehungsweise API-Account bei OpenAI oder Anthropic.

## Ein Feature, sauber umgesetzt

Dieses Update konzentriert sich nur auf Intake AI. Die Core-App funktioniert weiterhin ohne KI und ohne Abo, und Intake AI soll das Loggen beschleunigen, während die finale Entscheidung bei dir bleibt.
Intake AI ist ein optionales Add-on, gedacht für alle, die sich das Feature schon lange gewünscht haben. Zum Start kannst du entscheiden, ob
du Intake AI, einen eigenen API-Schlüssel oder gar keine KI nutzen möchtest. Wie immer liegt die Entscheidung vollständig bei dir. Keine KI zu nutzen, bedeutet keinerlei Einbußen bei anderen Features der App.

Das komplette Changelog findest du wie immer [hier](https://featurevoting.tobibechtold.dev/app/intake/changelog).

Vielen Dank, dass du Intake nutzt. Ich hoffe, dir gefällt das neue Release.

Tobi
