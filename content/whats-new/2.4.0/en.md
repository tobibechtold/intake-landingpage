---
version: "2.4.0"
publishedAt: "2026-06-21"
title: "What's new in Intake 2.4.0"
summary: "Intake AI is here"
coverImage: "./assets/cover.svg"
highlights:
  - Log meals with Intake AI
  - Describe meals in natural language
  - Analyze meal photos
  - Scan nutrition labels
  - Bring your own API key
---

## Intake AI is here

Intake 2.4.0 introduces Intake AI: a faster way to turn real meals into editable calories and macros.

You can describe what you ate, use a meal photo, or scan a nutrition label. Intake AI creates a suggestion you can review, adjust, and save to your diary.

![sharing](assets/ai.mp4)

## Three ways to start

Describe a meal in a sentence when typing is fastest. This works well for everyday meals, snacks, and quick corrections.

Use a photo when the meal is easier to show than explain. Intake AI can estimate a plate, snack, or restaurant meal and turn it into editable nutrition values.

Scan a nutrition label when you have packaged food in front of you. Intake AI can read the label and prefill the values before you save anything.

![sharing](assets/nutrition.mp4)

## Review before saving

AI results are estimates and can be wrong. Intake AI does not silently write values into your diary.

Every result stays editable. If something looks off, change the values directly or reply naturally in the chat, for example with a different portion size or a missing ingredient.

## Pricing

Unfortunately, there is no way around offering Intake AI as a subscription model. Every photo and every conversation with AI creates costs,
and those costs can only be covered through a monthly or yearly amount. Intake's "no subscription" promise still applies:
For Intake's core functionality, the rule remains the same: pay once, use it forever. I hope you understand that I cannot build a feature
where I end up paying money so you can use it.

## Bring your own API key

You can also use Intake AI with your own OpenAI or Claude API key. Add the key in Intake's settings, choose a supported model, and Intake sends AI requests through that provider instead of requiring an Intake AI subscription.

Provider terms and provider-side usage costs still apply. Intake does not control those prices or billing rules.

Important: a normal ChatGPT Plus, ChatGPT Pro, Claude Pro, or Claude Max subscription does not include an API key or API credits. API access is separate and requires a developer/API account with OpenAI or Anthropic.

## One feature, built carefully

This update is focused on Intake AI only. The core app still works without AI and without a subscription, and Intake AI is meant to speed up logging while keeping the final decision in your hands.
Intake AI is an optional add-on for everyone who has wanted this feature for a long time. At launch, you can decide whether
you want to use Intake AI, your own API key, or no AI at all. As always, the decision is fully yours. Not using AI does not limit any other features in the app.

You can find the full changelog [here](https://featurevoting.tobibechtold.dev/app/intake/changelog).

Thank you for using Intake. I hope you enjoy the new release.

Tobi
